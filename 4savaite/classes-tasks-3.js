...
console.groupCollapsed('https://edabit.com/challenge/ifDM26p25bqS8EsFu');
{
  class Player{
    ...
    get hpPerc(){
      return Math.round(100 ** 2 * this.#hp / this.#maxHp) / 10 ** 2;
    }
    ...
    set en(value) {
      if(value > this.#maxEn) this.#hp = this.#maxEn;
    ...
    learnSkill = (skillName, stats) =>{
      this[skillName] = function(target) {
        ...
        if(this.en < stats.cost){
          return `${this.name} attempted to use ${skill.name}, but didn't have enough energy!`
        }else {
          this.en -= stats.cost;
        }
        ...
        let returnString = `${this.name} used ${skill.name}), ${stats.desc}, against $targetr.name}, doing ${calculatedDamageRounded} damage!`;
        ...
        if(target.Hp <= 0){
          ...
          returnString += ` ${target.name} is at $targetr.hpPerc}% health.`
        }
        return returnString; 
      }
    }
